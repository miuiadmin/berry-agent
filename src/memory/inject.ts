...
/** 通用化纪律句（§9.1 第 4 件〔2026-09-08 消化批强化〕——写做什么/为什么/怎么验，不写模型癖性自述；「怎么验」须可机械自检〔挂 §11.7 自检环——可数核对、可判定禁令、二元阈值，不可机械复核的怎么验不是验证是愿望〕〔2026-09-19 五篇研究批 B——§11.7「怎么验」独立结构段定形注：锚定物随技能文件结构化携带非仅晋升时点检查〕；技能省的是复现成本，不期失败→成功翻转） */
const PROMOTION_DISCIPLINE =
  '写技能时写「做什么 / 为什么 / 怎么验」，不写模型癖性自述；「怎么验」须可机械自检（可数核对、可判定禁令、二元阈值）且以独立「怎么验」段承载（验证锚定物随技能文件携带、晋升后可回验），修改范围尽量小——技能省的是复现成本，不期把失败任务翻成成功。';

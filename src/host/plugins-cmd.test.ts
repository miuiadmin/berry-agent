...
describe('plugins list——同构装载态清单（三分区）', () => {
  ...
  it('缺省装载形：空目录 = core 内置态全装（批 19a—19d——exec/web/skills/memory/subagent/scheduler/mcp/browser/lsp/goal/checkpoint 入册，清单缺席）', async () => {
    const dir = tmpDir('plug-list-empty-');
    const io = capture();
    const code = await runPluginsEntry({ sub: 'list' }, { version: 'x', dataDir: dir, ...io });
    expect(code).toBe(0);
    const text = io.out.join('\n');
    // core 注册表非空（exec/web/skills/memory/subagent/scheduler/mcp/browser/lsp/goal/checkpoint 入册）——装载态集成回归锁（件数随逐纵切笔增长）
    expect(text).toContain('启用（11）：');

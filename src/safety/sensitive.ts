 ...
 * host/allowlist-store.ts 的 ALLOWLIST_BASENAME。safety 不 import
 ...
 * - allowlist.json：免问面（04 §9——免问条目本体泄出即审批面测绘）。
 * canonical 派生不可省：literal deny（seatbelt）与 ro-bind 遮蔽（bwrap）
 * 都必须落在符号链解析后的真实路径上才生效（/tmp → /private/tmp 字面
 * miss——2026-09-08 实机核验定形）。
 */
export const SENSITIVE_READ_BASENAMES: readonly string[] = ['secret.key', 'allowlist.json'];

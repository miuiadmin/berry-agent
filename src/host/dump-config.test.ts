...
describe('runDumpConfigEntry 同构诊断（07 §5 :memory: 纪律）', () => {
  it('好形：JSON 全形输出（database :memory: + 真数据目录 + 主库零落盘 + 不占标记）', async () => {
    ...
    // 批 19a 起 core 注册表非空（exec/web/skills/memory/subagent/scheduler
    // 入册——19c-2 第六件）——清单缺席 = 全 core 内置态：activated 见 core 行
    // （装载态集成回归锁——件数随逐纵切笔增长）。:memory: 座上 sqlite() 在场
    // → memory/scheduler 件照装（同构诊断形走真装载面——禁侧门律；引擎构造
    // 不自启——诊断进程不起钟零定时器残留）
    ...
    expect(doc.counts).toEqual({ total: 6, enabled: 6, failed: 0 });
  ...
  it('同构真读（核心回归锁）：enabled.yaml 磁盘行无账本 → failed 面见「装机账本无此 id」', async () => {
    ...
    // core 注册表非空（批 19a—19c-2——exec/web/skills/memory/subagent/scheduler 入册）与

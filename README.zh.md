...
> 状态：`0.1.0-alpha.11`——契约先行、逐批纵切落地；1.0 前 API 面仍可能调整。
...
AGI（通用人工智能）正在逼近；它身后等着 RSI（递归自我改进）——系统改进自身、
而改进又让「改进本身」更强的正反馈循环。berry-agent 两者都不是，也不假装是。
它是给这个时代真正的问题一个工程回答：

**当智能足够便宜、足够通用，谁来替你值守？**

所以我们把自进化当产品原则来交付，而不是宣言。它从今天就可验证的有界闭环
开始——会话沉淀为记忆、经验沉淀为技能、每一步运行都可回放可审计——而不是
重写自身的宏大叙事。基座强在接口、能力长在插件、进化只碰数据：安全判据面
（开门制、预算护栏、审计时间线）属宿主裁决权，永远在进化的射程之外。

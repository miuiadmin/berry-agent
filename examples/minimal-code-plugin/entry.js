 ...
 *
 * 契约要点（完整能力面见 docs/plugin-development.md）：
 ...
 * - 每个注册动词返回 disposer 或记入 ctx.effect（回卷律——见
 *   docs/plugin-development.md）——本模板取 ctx.effect 形：包住的注册在换代/
 *   unmount 时自动回卷（丢弃 disposer 不接 effect = 跨换代注册残留，属插件
 *   侧契约违例）。
 */
...
export default async (ctx) => {
  ...
  ctx.effect(() =>
    ctx.channels.registerCommand(
      'example-hello',
      (args) => {
        // handler 单参形（命令契约定形——见 docs/plugin-development.md）：
        // args.raw = 命令名后的输入原文（引号形态原样）、
        // args.argv = 引号感知切分的词数组。
        const text = args.raw.trim() === '' ? '（无参）' : args.raw;
        // ctx.ui.notify = 一次性通知原语（无会话位恒可——档位语义见
        // docs/plugin-development.md ctx 能力面表）

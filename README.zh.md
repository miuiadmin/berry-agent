...
| **模型无关**         | 由 [pi-ai](https://github.com/earendil-works/pi-ai) 供能：Anthropic、OpenAI、Google 等统一一面。一个环境变量换模型，零代码改动、零锁定。 |
...
| **零遥测**           | 无使用统计、无崩溃上报、无版本检查外呼。缺省网络面 = 模型调用 + 你显式要的动作，此外零。                                                 |
...
berry-agent 的机制层承接 berry、pi、dsh、opencode 已验证的思路——学术根在
Emacs「本体即解释器」与 VS Code 的特权双轨教训。模型调用由
[pi-ai](https://github.com/earendil-works/pi-ai) 供能。代码本身是从零的独立重写。

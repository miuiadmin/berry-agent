...
export interface SandboxPickEntry {
  /** 档位词（三档之一——onSelect 原样回传） */
  readonly mode: string;
  /** 行说明（右段——档位语义短注；danger 行警示语规范钉死） */
  readonly detail: string;
}

/** 沙箱档位选择器装配选项 */
export interface SandboxPickerOptions {
  /** 条目快照（三档序由装配保证——本件原样呈现） */
  readonly entries: readonly SandboxPickEntry[];
  /** 当前档（● 标记判据——会话 fold 现值；undefined = 诚实无锚） */
...
/**
 * 沙箱档位选择器内容件（OverlayContent——副屏 root 直收全屏 region；自持
 * 光标与视口窗口，ThemePicker/ThinkingPicker 同基建）。快照档——构造后静态，
 * 返回主屏全帧补显。
 */
export class SandboxPicker implements OverlayContent {
  private readonly entries: readonly SandboxPickEntry[];
  private readonly current: string | undefined;
  private readonly onSelect: (mode: string) => void;
  ...
  constructor(options: SandboxPickerOptions) {
  ...
  render(buffer: CellBuffer, region: Region): void {
    ...
    if (this.entries.length === 0) {
      buffer.writeText(region.row + 1, region.col, '（无档位条目——三档词表恒在，此形属装配防御位）', HINT_STYLE);
    } else {
  ...
  private renderRow(buffer: CellBuffer, row: number, col: number, width: number, index: number): void {
    ...
    const mark = this.current !== undefined && entry.mode === this.current ? `${CURRENT_MARK} ` : '  ';
    const left = `${prefix}${mark}${entry.mode}`;
    // 右段预算律单源：警示语先按预算 … 截断再右对齐（窄窗不再负起列劈毁档名）
    ...
    if (k !== null && k.phase !== 'release') {
      ...
      if (this.entries.length > 0) {
        ...
        if (isPlainKey(k, 'enter')) {
          const chosen = this.entries[this.cursor]!.mode;

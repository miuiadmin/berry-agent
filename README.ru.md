...
**Единый расширяемый персональный агент для эры AGI — создан работать без присмотра.**
...
**4 000+** тестов · **6** проверяемых машиной контрактов релиза · **0** телеметрии

> Статус: `0.1.0-alpha.1` — контракт-первая разработка, постройка вертикальными
...
| **Врата возможностей, а не «на глазок»** | Опасные возможности живут за явными вратами — `berry-agent doors list` показывает состояние каждых. Установка плагина никогда не означает наделения его правами.                                                |
| **Независимость от модели**              | На базе [pi-ai](https://github.com/earendil-works/pi-ai): Anthropic, OpenAI, Google и другие за одним интерфейсом. Смена модели одной переменной окружения — без правок кода, без привязки.                     |
...
| **Нулевая телеметрия**                   | Никакой статистики использования, никаких отчётов о сбоях, никаких «звонков домой» за версией. Сеть по умолчанию — вызовы модели плюс то, что вы явно запросили. Ничего сверх.                                  |
...
```bash
berry-agent                    # TUI: сразу в диалог (продолжает последнюю сессию текущего каталога)
berry-agent run "разовая задача"  # одиночный запуск → stdout
berry-agent sessions list      # сессии: list / resume / fork / search / reindex
berry-agent plugins list       # плагины: list / check / install / uninstall / mount / unmount / toggle / update
berry-agent credentials list   # учётные данные: add / list / rm (в TUI есть и OAuth-поток)
berry-agent doors list         # состояние врат возможностей (только чтение)
berry-agent serve --port 7860  # резидентный хост: веб-интерфейс + программная поверхность /v1/*
```
...
Все включены по умолчанию; каждый можно отключить по отдельности.
...
- **HTTP** — `berry-agent serve` запускает резидентный хост с веб-интерфейсом и
...
- **SDK** — типизированный TypeScript-клиент (spawn stdio или прямой HTTP) живёт
  в репозитории; npm-пакет `berry-agent-sdk` выйдет вместе с бетой.
- **MCP** — `berry-agent mcp` выставляет агента как MCP-сервер, так что любой
...
graph TD
    ...
    PLUGINS["Поверхность плагинов<br/>13 глаголов расширения · 16 встроенных плагинов"]
...
Механический слой berry-agent несёт вперёд проверенные идеи **berry**, **pi**,
**dsh** и **opencode** — с академическими корнями в эмаксовском «редактор — это
интерпретатор» и уроке VS Code о привилегированных двойных путях. Доступ к LLM
обеспечивает [pi-ai](https://github.com/earendil-works/pi-ai). Сам код —
независимая переработка с нуля.

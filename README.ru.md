...
> Статус: `0.1.0-alpha.2` — контракт-первая разработка, постройка вертикальными
...
Обновляетесь с alpha.1? Имя бинарного файла сменилось на `berry`; старая команда
`berry-agent` по-прежнему доступна как запасная, а при обновлении до версии с
новым именем bin ссылка автоматически переключится на `berry`.
